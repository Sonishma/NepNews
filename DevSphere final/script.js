...
async function loadApprovedNews() {
    try {
      const response = await fetch('http://localhost:5000/news/approved-news'); // Fetch approved news
      const newsData = await response.json();
  
      const newsContainer = document.querySelector('.news-container');
      newsContainer.innerHTML = ''; // Clear previous content
  
      // Create a full news container (for full article view)
      ...
      newsData.forEach(news => {
        ...
        // Read more toggle (individual card toggle)
        ...
        // Full view on news item click
        newsItem.addEventListener("click", function (event) {
          // Prevent triggering if read-more was clicked
          if (event.target.classList.contains("read-more")) return;
  
          const title = news.newsTitle;
          const imgSrc = "https://via.placeholder.com/150";
          const description = news.newsDescription;
          const timeCountry = `Published by ${news.author} on ${new Date(news.date).toLocaleDateString()}`;
          ...
          fullNewsContainer.innerHTML = `
            ...
            <div class="full-news-content">
              ...
              <!-- Popup HTML inside the fullNewsContainer -->
  <div id="popup-ad" class="popup-ad">
    <div class="popup-content">
      <button id="close-popup" class="close-btn">&times;</button>
      <a href="https://www.booking.com/index.en-gb.html?aid=2369666..." target="_blank">
        <img src="http://localhost:5000/uploads/1747234988889.jpg" alt="Booking.com Advertisement">
      </a>
    </div>
  </div>
          `;
  ...
  }, 0); // Wait for DOM to paint first

  // Back button logic
  fullNewsContainer.querySelector("#back-btn").addEventListener("click", function () {
    fullNewsContainer.style.display = "none";
  ...
  // Optional category-based filtering function
  function loadCategory(category) {
    const categories = document.querySelectorAll('.news-category');
  ...
  document.addEventListener("DOMContentLoaded", function () {
    loadApprovedNews(); // Load news on page load
  });
  ...
  document.addEventListener('DOMContentLoaded', () => {
    fetch('http://localhost:5000/api/ads/top')
      .then(response => {
        if (!response.ok) {
          throw new Error('Failed to fetch top ad');
        }
      ...
      .then(ad => {
        ...
        // Set the ad image source
        imageElement.src = ad.imageUrl.startsWith('http') 
          ? ad.imageUrl 
          : `http://localhost:5000${ad.imageUrl}`;
  
        // Optionally make the image clickable (linking to ad)
        if (ad.websiteLink) {
          const wrapper = imageElement.closest('.nav-ad-container');
        ...
        if (ad.title) {
          labelElement.textContent = ad.title;
        }
      })
      .catch(error => {
        console.error('Error loading top ad:', error);
      });
  });
  fetch('http://localhost:5000/api/ads/bottom')
    ...
    const newsData = [
      {
        newsTitle: "Football League Finals This Weekend",
        newsDescription: "The grand finale of the football league is happening this Saturday...",
        author: "ESPN",
        date: "2024-04-20"
      },
      {
        newsTitle: "NASA Discovers New Exoplanet",
        newsDescription: "NASA's James Webb telescope has discovered a potentially habitable planet...",
        author: "NASA",
        date: "2024-03-10"
      }
    ];
      ...
      newsItem.addEventListener('click', function (e) {
        ...
        fullNewsContainer.innerHTML = `
          ...
          <div class="full-news-content">
            ...
            <img src="https://via.placeholder.com/150" alt="News Image">
            <p class="time-country">Published by ${news.author} on ${new Date(news.date).toLocaleDateString()}</p>
            <p>${news.newsDescription} More details about the news will appear here...</p>
          </div>
          <div id="popup-ad" class="popup-ad">
            <div class="popup-content">
              <button id="close-popup" class="close-btn">&times;</button>
